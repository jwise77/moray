...
recombinations balnace photo-ionizations.  He derived a radius of an
\ion{H}{2} region,
%
\begin{equation}
  \label{eqn:Rstr}
...
studied extensively were champange flows.  Here the source is embedded
...
\item Ray tracing---Radiation can be propagated along rays that extend
  through the compuational grid \citep[e.g.][]{Razoumov99, Abel99_RT,
...
three-dimensions.  In the second comparision paper \citep[hereafter
...
rays.  We use the terms photon packages and rays interchangably.  Next
we focus on the details of the radiation hydrodynamics and then the
importance of correct timestepping.  Last we give our parallelization
...
occur in champange flows when the ionization front passes a density
...
numerical solution evenutally approaches the analytical solution.  If
...
shadowing created by a dense, opticlally thick clump, and (4) multiple
...
$L_{\rm box} = 1.5$ kpc; and (2) a blastwave is initialized in the
center of the domain.  We use 3 levels of AMR with a base grid of
...
fractions and tempeatures in the ambient medium.  In effect, a
...
\begin{figure}[t]
  \epsscale{0.75}
  \plotone{fig/vary_dt}
  \caption{\label{fig:dt_dep2} Variable timestepping for the methods
...
resolution to compare different timestepping methods --- restricted
...
the \ion{H}{2} restricted and incident radiation variable timestepping
...
The variable timestepping of the $dn_{\rm H}/dt$ and $dI/dt$ methods
...
more dynamical problems---champange flows, Rayleigh-Taylor
