...
\affil{Department of Astrophysical Sciences, Princeton University,
  Peyton Hall, Ivy Lane, Princeton, NJ 08544}

\author{Ji-hoon Kim and Tom Abel} 
...
\begin{abstract}
  ...
  refinement (AMR) code, Enzo.  By coupling the solver with the energy
  ...
  includes an expanding \ion{H}{2} region in a magnetized medium,
  utilizing the newly implemented magnetohydrodynamics module in
  \enzo.  This method linearly scales with the number of point sources
  and number of grid cells.  To combat this, we use a novel method of
  merging rays at large radius, using a tree method, which we briefly
  describe.  Our implementation is scalable to 512 processors on
...
\ion{H}{2} regions in spherical symmetry and plane-parallel ionization
...
fronts are subsonic.  Furthermore ``weak'' and ``strong'' R-type fronts
...
expanding \ion{H}{2} illuminated by a massive star in a uniform
medium:
%
\begin{enumerate}
...
\item Strong and weak D-type---The front continues to slow, becoming a
  ...
  front.
\item Expansion phase---After the shock forms, the \ion{H}{2} region
...
in an overdense region, and the \ion{H}{2} region escapes from this
...
\item Moment methods---The angular moments of the radiation field can
  ...
  Moment methods have the advantage of being fast, being independent
  of the number of radiation point sources.  However, they are
...
growth of an \ion{H}{2} region from a 100\Ms~Population III (Pop III)
star \citep{Abel07}, the early stages of reionization from Pop III
stars \citep{Wise08_Reion}, radiative feedback on the formation of
high redshift dwarf galaxies \citep{Wise08_Gal}, ultraviolet radiation
...
dilution of the radiation.  Thus equation (\ref{eqn:rteqn}) reduces to
the non-cosmological form in this local approximation,
%
\begin{equation}
  \frac{1}{c} \frac{\partial I_\nu}{\partial t} + 
...
trilinearly interpolated densities \citep[see][]{Mellema06} but could
not devise a method that did not produce considerable artifacts.
Equation (\ref{eqn:rtray}) has a simple exponential analytic solution,
and the photon flux of a ray is reduced by
%
\begin{equation}
  \label{eqn:flux}
  dP = P \times (1 - e^{-\tau})
...
photo-ionization rates are better utilized in solvers that consider
chemical networks \citep[e.g.][]{Abel97}.
...
well-sampled by the rays.  The main source of these artifacts is the
geometrical difference between the cell and the HEALPix pixel.  In
this section, we devise a correction scheme to account for these
...
through the two-step Solomon process that are composed of 76
absorption lines ranging from 11.1 to 13.6 eV \citep{Stecher67,
  Dalgarno70}.  Each of these spectral lines can be modelled with a
typical exponential attenuation equation \citep{Ricotti01}, but
...
\ion{He}{2} is negligible.  The number of secondary ionizations of H
...
likely to have high absorption cross-sections (e.g. \ion{H}{1}
ionizations near 13.6 eV) or a low initial intensity (e.g. \ion{He}{2}
...
\citep[e.g.][for hydrogen ionizing radiation only]{Abel07,
  Wise08_Gal}, we have 3 energy groups---\ion{H}{1}, \ion{He}{1},
\ion{He}{2}---each with an energy that equals the average photon
energy above the ionization threshold.
...
traced through the cells.  In the case of a single \ion{H}{2} region,
...
\ion{H}{2} regions in a cosmological density field.  In all of the
...
in RT06.  In Figure \ref{fig:test2_2}, we show the ratio of the
...
codes in the upper range of RT06.  In Figure \ref{fig:test2_3}, we
show slices of neutral fraction and temperature through the origin at
$t = $ 10 and 100 Myr.  Here one sees the spherically symmetric
\ion{H}{2} regions and a smooth temperature transition to the neutral
ambient medium.
...
expansion of an \ion{H}{2} region in a uniform medium, similar to Test
2, (2) an \ion{H}{2} region in an isothermal sphere, and (3) the
...
\ion{H}{2} with a variable source.  For the \citeauthor{Iliev09}
...
problem of a growing \ion{H}{2} region in a magnetic field.
...
direction.  At $t = 10$ kyr, the surface of the \ion{H}{2} region is
...
reflecting.  These will cause features that are non-physical, in
particular the top reflecting boundary; nevertheless, these tests
provide a good check on a radiation hydrodynamics solver.  We show the
evolution of the density, temperature, and ionized fraction of the
...
\citep{Whalen08_Instab} that are seen on the \ion{H}{2} region surface
...
\begin{figure}[t]
  \epsscale{1.15}
  \plottwo{fig/test12_kph}{fig/test12_radial}
...
\begin{figure}[t]
  ...
  \caption{\label{fig:test12_profiles} Test 12 (Time variations of the
    ...
    in the source have little effect on the overall \ion{H}{2} region
...
the overall growth of the \ion{H}{2} region.  It has the approximately
...
As the \ion{H}{2} region grows in the magnetized medium, shown in
...
\begin{figure}[t]
  \epsscale{1.05}
  \plottwo{fig/nures040}{fig/nures100}
...
We use Test 5 (\S\ref{sec:test5}; an expanding \ion{H}{2} region with
...
Figure \ref{fig:nu_dep} shows the radial profiles of density,
temperature, ionized fraction, and radial velocity at $t = 200$ Myr
...
a marginally higher density in the outer shock and lower ionized
...
large-scale quantities, e.g. ionized filling fractions in reionization
...
\begin{figure}[t]
  \epsscale{0.75}
  \plotone{fig/ifront_dt}
  \caption{\label{fig:dt_dep1} Growth of the ionization front radius,
...
\begin{figure}[t]
  ...
  \caption{\label{fig:dt_dep2} Variable time-stepping for the methods
    ...
    mean free path by the ionization front is plotted for reference.}
  \epsscale{1}
\end{figure}
...
to an accurate \ion{H}{2} radii at late times.  Even $dt_P = 5$ Myr
...
evolution of the \ion{H}{2} is matches the analytical expectations
...
increased ionization fractions and temperatures in the \ion{H}{2}
region.
...
\begin{figure}[t]
  \epsscale{0.6}
  \plotone{fig/two_sources.eps}
  \caption{\label{fig:twosrc} Slice of neutral fraction at $t = 500$
    Myr through the sources in the consolidated \ion{H}{2} test.
...
Overall it scales well to 512 processes, and we plan to enhance the
weak scalability of Enzo+Moray in the near future to larger processor
counts.
...
time-varying source, and an \ion{H}{2} region with MHD---to
...
combination of AMR and adaptive ray tracing allows for high-resolution
and high-dynamical range problems, e.g. present-day star formation,
molecular cloud resolving cosmological galaxy formation, and
\ion{H}{2} regions of Population III stars.  Furthermore, we have
included Lyman-Werner absorption, secondary ionizations from X-ray
radiation, Compton heating from photon scattering, and radiation
...
version of \enzo\footnote{http://lca.ucsd.edu/enzo}, providing the
