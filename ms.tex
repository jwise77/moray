...
period.  The box has 675 point sources, ??? AMR grids, and ???
computational cells in this calculation at this redshift.  In the
first radiative transfer timestep, ??? and ??? ray segments are traced
with and without ray merging.  The ionized volume fraction is ???.
The calculations are performed on the Nehalam nodes on Pleiades on
2$^n$ cores, where $n = [2,...,8]$.
