...
flows.  We use Test 5 (\S\ref{sec:test5}; an expanding \ion{H}{2}
...
the \ion{H}{2} restricted and incident radiation variable time-stepping
...
analytical one, but they converge to an accurate \ion{H}{2} radii at
...
ionization fractions and temperatures in the \ion{H}{2} region.
...
\begin{figure}[t]
  ...
  \caption{\label{fig:strong} Strong scaling test with a $256^3$ AMR
    ...
    measures load balancing.  The hydrodynamics and non-equilibrium
    chemistry solvers scale well.  Unfortunately the communication of
    rays does not scale in this problem, where as the ray tracing
...
3$ Mpc/h, a resolution of $256^3$, and six levels of refinement.  We
measure the time spent on the hydrodynamics, non-equilibrium
chemistry, ray tracing, and radiation transport communication in a
timestep lasting 1 Myr, at $z=10$.  There are nine radiative transfer
timesteps in this period.  The box has 675 point sources, ??? AMR
grids, and ??? computational cells in this calculation at this
redshift.  In the first radiative transfer timestep, ??? and ??? ray
segments are traced with and without ray merging.  The ionized volume
fraction is ???.  The calculations are performed on the Nehalam nodes
on Pleiades on 2$^n$ cores, where $n = [2,...,8]$.  
...
scheme to increase the feasbility of running larger problems with the
continually increasing computational resources.
...
cloud resolving galaxy formation simulations and \ion{H}{2} regions of
